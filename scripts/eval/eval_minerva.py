...
import sys
from utils import get_filtered_json_files
def eval_minerva_import(tokenizer_path, jsonl_file_path):
    ...
    if os.path.isdir(jsonl_file_path):
        ...
        json_files = get_filtered_json_files(jsonl_file_path, 'samples_minerva_algebra')
    else:
    ...
    return max(acc_list), sum(avg_tokens_list) / len(avg_tokens_list)
