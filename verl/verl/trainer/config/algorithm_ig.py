...
@dataclass(frozen=True)
class GRPOWeightingConfig(BaseConfig):
    """Configuration for GRPO weight reallocation.

    Args:
        enable (bool): Whether to enable GRPO reweighting.
        beta (float): Sigmoid sensitivity for entropy gating.
        entropy_clip (float): Clamp for standardized entropy.
        eps (float): Numerical stability term.
        correct_threshold (float): Threshold to binarize correctness from score.
    """

    enable: bool = False
    beta: float = 1.0
    entropy_clip: float = 5.0
    eps: float = 1e-6
...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        grpo_weighting (GRPOWeightingConfig): GRPO reweighting configuration.
    """
    ...
    grpo_weighting: GRPOWeightingConfig = field(default_factory=GRPOWeightingConfig)
