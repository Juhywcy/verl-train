        ...
        if truncation_value is not None:
             ...
             if truncation_mask is not None:
                 ...
                 if top_k is not None and top_k > 0:
                     ...
                     # Clone probs to avoid modifying saved tensor
                     probs_for_sort = probs.clone()
                     # Set label probability to -1 (or very small) so it's not selected as top-k unsampled
                     probs_for_sort.scatter_(-1, labels.unsqueeze(-1), -1.0)
                     
                     # Get top-k indices
                     _, topk_indices = torch.topk(probs_for_sort, top_k, dim=-1) # (N, k)
                     # Create top-k mask
                     # topk_mask = torch.zeros_like(mask, dtype=torch.bool).expand_as(grad_logits) # (N, V)
                     topk_mask = torch.zeros(grad_logits.size(), dtype=torch.bool, device=grad_logits.device)
                     topk_mask.scatter_(-1, topk_indices, True)
                     
                     # Combine with sample-level mask
                     # Final mask: (Sample is Positive) AND (Token is in Top-K Unsampled)
                     final_mask = mask & topk_mask
                 else:
                     final_mask = mask

                 grad_logits = torch.where(final_mask, grad_logits_clamped, grad_logits)
             else:
